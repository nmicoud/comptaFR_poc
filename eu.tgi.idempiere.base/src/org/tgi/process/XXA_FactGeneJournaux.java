 ...
 *  @author Nico
 */
public class XXA_FactGeneJournaux extends ComptaFrProcess
{
	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++) {
			...
			if (para[i].getParameter() == null && para[i].getParameter_To() == null)
				...
				p_DateAcct_To = para[i].getParameter_ToAsTimestamp();	// Si uniquement date de fin, le paramètre n'est pas pris en compte
			}
			else if (name.equals("GL_Category_ID"))
				p_GL_Category_ID = para[i].getParameterAsInt();
			else if (name.equals("XXA_IsGroupByRecord"))
				p_isGroupByRecord = para[i].getParameterAsBoolean();
			else if (name.equals("XXA_IsAccountDetail"))
	...
	protected String doIt () throws Exception
	{
		...
		int sequenceID = MTXXAReport.getSequenceID(get_TrxName());
		String footerCenter = "";
...
//		}
		...
		if (p_isGroupByRecord)
			footerCenter = Msg.getMsg(getCtx(), "XXA_EcrituresGroupeesParCompte");
		...
		for (int glCatID : glCatIDs) {
			...
			MTXXAReport taf = null;
			String printName = "";
			if (!Util.isEmpty(code))
				printName = "(" + code + ") ";
			printName += affich;

			String criteresDate = MTXXAReport.getDateCriteres(getCtx(), getAD_Client_ID(), p_DateAcct_From, p_DateAcct_To);

			String sql2 = "";
			if (!isJournalCent) {
				reportTitle = Msg.getElement(getCtx(), "GL_Category_ID") + " " + affich + (Util.isEmpty(code) ? "" : " (" + code + ")");

				String s_insert = "INSERT INTO T_XXA_Report "
						+ "(T_XXA_Report_ID, AD_PInstance_ID, AD_Client_ID, AD_Org_ID, Created, CreatedBy, Updated, UpdatedBy, "
						+ " Fact_Acct_ID, BPName, AccountValue, GL_Category_ID, AmtAcctDr, AmtAcctCr, DateAcct, Description,"
						+ " XXA_FactAcctOrg, Account_Name, PrintName, ClientName, OrgName, FooterCenter, Title, XXA_DateAsString/*, XXA_NumEcriture*/)"; // FEC

				if (p_isGroupByRecord) {
					int dt_bs = MTXXAReport.getDocTypeForBankStatement(getCtx());
					int dt_a = MTXXAReport.getDocTypeForAllocation(getCtx());
			...
			if (!isJournalCent) // on remplit la table par du sql
				...
				while (rs2.next()) {
					taf = new MTXXAReport(getCtx(), 0, getAD_PInstance_ID(), get_TrxName());
					...
					taf.setXXA_DateAsString(criteresDate);
		...
		statusUpdate("# Fin du traitement ; " + lines + " lignes générées : " + lines + " en " + duree); // translate
		return "# Fin du traitement ; " + lines + " lignes générées : " + lines + " en " + duree; // translate
	}	//	doIt

}	//	XXA_FactGeneJournaux